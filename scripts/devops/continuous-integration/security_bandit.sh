...
readonly ALLOWED_CONFIG_FILES=(".bandit" "bandit.yaml" "pyproject.toml")
readonly TOOL="bandit"
...
show_usage() {
    ...
    logger "LINK" "https://${TOOL}.readthedocs.io/en/latest/config.html"
...
main() {
    ...
    if [ "$status" -eq 0 ]; then
        logger "INFO" "🎉🎉🎉 ${TOOL} security check passed."
    else
        logger "ERROR" "💥💥💥 ${TOOL} security check failed."
    fi
