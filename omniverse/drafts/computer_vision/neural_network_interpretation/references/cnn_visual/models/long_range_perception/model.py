...
def conv_act_max(act="relu", *args, **kwargs):
    activation = nn.ModuleDict({"relu": nn.ReLU(), "lrelu": nn.LeakyReLU()})[act]

    return nn.Sequential(nn.Conv2d(*args, **kwargs, kernel_size=3, padding=1), activation, nn.MaxPool2d(kernel_size=2))
